...
def main():
    ...
    counter = 0
    threads = [threading.Thread(target=increment_counter, args=(f"T{i}", False)) for i in range(5)]
    ...
    print("\n--- Running WITH Lock ---")
    counter = 0
    threads = [threading.Thread(target=increment_counter, args=(f"T{i}", True)) for i in range(5)]
