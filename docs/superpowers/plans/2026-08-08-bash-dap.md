...
def test_launch_body_defaults():
    ...
    body = profile.adapter.launch_body(
        program="/tmp/x.sh", args=["a"], cwd="/tmp", env=None,
        stop_on_entry=True, console="internal", opts={},
    )
    assert body == {
        "type": "bash", "request": "launch", "program": "/tmp/x.sh",
        "args": ["a"], "cwd": "/tmp", "stopOnEntry": True,
    }
...
def test_launch_body_env_and_bash_override():
    ...
    body = profile.adapter.launch_body(
        program="/tmp/x.sh", args=[], cwd="/tmp", env={"K": "V"},
        stop_on_entry=False, console="internal", opts={},
    )
    assert body["env"] == {"K": "V"}
...
_DECLARE_RE = re.compile(r"^declare\s+-([-aAilnrtux]+)\s+([A-Za-z_][A-Za-z0-9_]*)(=(.*))?$")
        ...
        *,
        ...
        try:
            ...
            if isinstance(e, BashProtocolError):
                raise
            raise BashProtocolError(
                "the bash harness never reported ready — is bash hung "
                "during startup?"
            )
...
async def launch_stopped(client: AdapterClient, program: str, breakpoints=None,
                         stop_on_entry=True):
    ...
    launch_fut = client.send("launch", {
        "type": "bash", "request": "launch", "program": program,
        "args": [], "cwd": str(Path(program).parent), "stopOnEntry": stop_on_entry,
    })
...
from tests.integration.bash_adapter_harness import (
    FIXTURES, bash_ok, launch_stopped, start_bash_adapter,
)
...
async def test_launch_missing_program_fails():
    client = await start_bash_adapter()
    try:
        fut = client.send("launch", {
            "type": "bash", "program": "/nonexistent/x.sh",
...
async def test_breakpoint_from_config_phase():
    ...
    try:
        ...
        await launch_stopped(client, program,
                             breakpoints=[{"line": 5}], stop_on_entry=False)
        ev = await client.wait_event("stopped")
        assert ev["body"]["reason"] == "breakpoint"
        frames = (await client.request(
            "stackTrace", {"threadId": 1}))["body"]["stackFrames"]
...
async def test_conditional_breakpoint_via_dap():
    ...
    try:
        ...
        result = (await client.request("evaluate", {
            "expression": 'echo "i=$i"', "context": "repl",
        }))["body"]["result"]
...
async def test_reset_breakpoints_while_stopped():
    ...
    try:
        program = str(FIXTURES / "bash_loop.sh")
        await launch_stopped(client, program,
                             breakpoints=[{"line": 3}], stop_on_entry=False)
        await client.wait_event("stopped")
        # replace with an empty set -> free run to exit
        await client.request("setBreakpoints",
                             {"source": {"path": program}, "breakpoints": []})
        await client.request("continue", {"threadId": 1})
        await client.wait_event("exited")
    finally:
        await client.stop()
...
async def test_step_in_and_out():
    ...
    try:
        ...
        await launch_stopped(client, program, breakpoints=[{"line": 6}],
                             stop_on_entry=False)     # outer(): `inner` call
        await client.wait_event("stopped")
...
async def test_next_stays_in_frame():
    ...
    try:
        program = str(FIXTURES / "bash_functions.sh")
        await launch_stopped(client, program, breakpoints=[{"line": 9}],
                             stop_on_entry=False)     # top level: `outer`
        await client.wait_event("stopped")
...
async def test_scopes_and_variables_with_array_children():
    ...
    try:
        ...
        await launch_stopped(client, program, breakpoints=[{"line": 4}],
                             stop_on_entry=False)
        await client.wait_event("stopped")
        frames = (await client.request(
            "stackTrace", {"threadId": 1}))["body"]["stackFrames"]
...
async def test_evaluate_mutates_debuggee():
    ...
    try:
        ...
        result = (await client.request("evaluate", {
            "expression": 'echo "x=$x"', "context": "repl"}))["body"]["result"]
...
class BashDapServer:
    ...
    def send_response(self, request: Request, body: dict | None = None) -> None:
        self._write(Response(seq=self._next_seq(), request_seq=request.seq,
                             command=request.command, success=True,
                             body=body or {}).to_dict())
    ...
    def send_event(self, event: str, body: dict | None = None) -> None:
        self._write(Event(seq=self._next_seq(), event=event,
                          body=body or {}).to_dict())
    ...
    async def _on_stackTrace(self, request: Request) -> None:
        ...
        frames = [
            {"id": i, "name": f["func"], "line": f["line"], "column": 1,
             "source": {"path": f["file"]}}
            for i, f in enumerate(self._stack_cache)
        ]
        self.send_response(request, {"stackFrames": frames,
    ...
    async def _on_scopes(self, request: Request) -> None:
        ...
        scopes = []
        if frame == 0:
            scopes.append({"name": "Locals", "expensive": False,
                           "variablesReference": self._add_ref(("scope", "locals"))})
        scopes.append({"name": "Globals", "expensive": False,
    ...
    async def _on_variables(self, request: Request) -> None:
        ...
        if entry is None:
            ...
            return
        if entry[0] == "scope":
            vars_ = (await self.session.locals() if entry[1] == "locals"
                     else await self.session.globals_vars())
            body = [self._var_to_dap(v) for v in vars_]
        else:
            body = [{"name": k, "value": val, "variablesReference": 0}
...
async def test_missing_bash_reports_hint():
    ...
    with pytest.raises(BashProtocolError, match="bash not found"):
        await session.launch(program="/tmp/x.sh", args=[], cwd="/tmp",
                             env=None, bash="definitely-not-bash-xyz")
...
async def test_old_bash_reports_version_error_fast(tmp_path):
    ...
    with pytest.raises(BashProtocolError, match="4.4"):
        await asyncio.wait_for(
            session.launch(program=str(FIXTURES / "bash_hello.sh"), args=[],
                           cwd="/tmp", env=None, bash=str(fake)),
