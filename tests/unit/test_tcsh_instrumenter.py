...
def test_literal_source_is_recursively_instrumented_in_depth_first_order(
    tmp_path: Path,
) -> None:
    ...
    assert str(generated_library) in [
        decode_literal_tcsh_word(word) for word in source_operands(generated)
    ]
    assert "# source-enter 1\n" in generated
    assert "# source-leave 1\n" in generated
    assert [
        (probe.id, probe.span.path, probe.span.start_line, probe.source_depth)
        for probe in result.source_map.probes
    ] == [
        (1, main_path, 1, 0),
        (2, library_path, 1, 1),
        (3, main_path, 2, 0),
...
def test_duplicate_sources_reuse_one_generated_file(tmp_path: Path) -> None:
    ...
    decoded_sources = [
        decode_literal_tcsh_word(word)
        for word in source_operands(result.root.read_text())
    ]
    assert decoded_sources.count(str(generated_library)) == 2
    assert len(result.source_map.probes) == 3


def test_source_path_with_spaces_is_rewritten_as_a_quoted_absolute_path(
    tmp_path: Path,
) -> None:
...
def test_structural_and_opaque_units_receive_no_probes_and_label_probe_follows_label(
    ...
    assert (
        generated.index("again:\n")
        < generated.index(first_probe_source)
        < generated.index("if (1) then")
    )
...
def test_only_eligible_lexical_dollar_zero_is_rewritten(tmp_path: Path) -> None:
    ...
    generated = result.root.read_text()
    assert (
        "echo ${__tcsh_dap_original_0} \"${__tcsh_dap_original_0}\" '$0' \\$0 # $0\n"
        in generated
    )
    assert "cat << EOF\n$0\nEOF\n" in generated
...
def test_original_argv0_rejects_nonrepresentable_characters(
...
def test_final_literal_source_is_separated_from_leave_event(tmp_path: Path) -> None:
    ...
    generated = result.root.read_text()
    assert "\n# source-leave 1\n" in generated
    assert decode_literal_tcsh_word(source_operands(generated)[1]) == str(
        generated_library
    )
...
def test_generated_script_executes_with_literal_paths_and_separated_fragments(
    tmp_path: Path,
) -> None:
    library = tmp_path / "lib.csh"
    library.write_text("echo library")
