...
class RustConcurrencyModal(ModalScreen[None]):
    ...
    def _render_threads(self) -> None:
        ...
        if self._snapshot.threads:
            ...
            self._render_thread_detail(self._snapshot.threads[index])
        else:
            self.query_one("#thread-evidence", Static).update("No Rust threads found")

    def _render_thread_detail(self, thread: ThreadAnalysis) -> None:
        content = Text()
        ...
        # The snapshot deliberately contains only immutable concurrency
        # observations. Actual stack-frame locals remain owned by the normal
        # controller navigation that SelectThread/SelectFrame opens.
        variables = self.query_one("#vars", VariableView)
        variables.clear()
        variables.root.add_leaf("Select to inspect live frame locals")
    ...
    def on_data_table_row_highlighted(self, event: DataTable.RowHighlighted) -> None:
        ...
        if row is not None and 0 <= row < len(self._snapshot.threads):
            self._render_thread_detail(self._snapshot.threads[row])
