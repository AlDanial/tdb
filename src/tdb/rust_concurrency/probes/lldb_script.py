...
def _rust_version(target) -> str | None:
    ...
    try:
        with open(path, "rb") as executable_file:
            with mmap.mmap(executable_file.fileno(), 0, access=mmap.ACCESS_READ) as data:
                match = _RUST_VERSION_BYTES.search(data)
...
def tdb_rust_snapshot(debugger, command, result, internal_dict) -> None:
    ...
    warnings: list[str] = []
    rust_version = _rust_version(target)
    if lldb is None or process.GetState() != lldb.eStateStopped:
